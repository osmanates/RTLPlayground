...
void parse_vlan(void)
{
	...
	if (!atoi_short(&vlan_settings.vlan, cmd_words_b[1])) {
		...
		if (cmd_compare(2, "mgmt")) {
			...
			// Stamp the CPU port's PVID with the management VLAN so
			// untagged frames generated by the uIP stack get tagged
			// on egress via trunk ports that are tagged members of
			// this VLAN. Without this, CPU-sourced ARP replies and
			// DHCP discovers go out with the default PVID (1), which
			// trunk ports drop because they aren't VLAN 1 members.
			// A value of 0 disables the management VLAN and restores
			// the CPU port PVID to 1 so plain untagged management on
			// the default VLAN keeps working.
			port_pvid_set(CPU_PORT, vlan_settings.vlan ? vlan_settings.vlan : 1);
